...
/**
  ...
  * int theTolerance how close thePoint can be to our line.
  */
boolean pointInsideLine(PVector thePoint, PVector theLineEndPoint1, PVector theLineEndPoint2, int theTolerance) {
  ...
  // inside distance determines the weighting between linePoint1 and linePoint2 
  float insideDistance = diff.dot(dir) / dir.dot(dir);

  if(insideDistance>0 && insideDistance<1) {
    // thePoint is inside/close to the line if insideDistance>0 or <1
    println( ((insideDistance<0.5) ? "closer to p1":"closer to p2" ) + "\t p1:"+nf((1-insideDistance),1,2)+" / p2:"+nf(insideDistance,1,2) );
    ...
    d.sub(closest);
    float distsqr = d.dot(d); // println((insideDistance>0.5) ? "b":"a");
